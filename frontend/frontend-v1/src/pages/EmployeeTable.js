...
function EmployeeTable() {
  ...
  useEffect(() => {
    ...
    if (searchStr.length === 0) {
      ...
      setSearchedEmployees(employees.filter((e) => e.empid.startsWith(searchStr.trim())));
    }
  ...
  const getSingleEmp = async (empid) => {
    // const reply = await employeeService.getEmployeeByEmpId(empid);
    // setEmp(reply.data);
  };
  ...
  const renderBody = () => {
    return (
      employees &&
      (searchedEmployees.length !== 0 ? searchedEmployees : employees).map((emp) => {
        return (
          <tr key={emp.id}>
            <td>{emp.empid}</td>
            <td>{emp.fName}</td>
  ...
  const MoreInfor = (id) => {
    history.push(`/employeemorinfor/${id}`);
